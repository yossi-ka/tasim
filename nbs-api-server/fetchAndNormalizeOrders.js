...
const NORMAL_PRODUCTS_SALE_ID = 8539;
const KFUIM_SALE_ID = 8566;
...
const getOrders = async (filtersObject, token, amountDaysToImport = 1) => {
  try {
...
const fetchAndNormalizeOrders = async () => {
  try {
    ...
    const paidFiltersObject = {
      searchTerm: "",
      saleIds: [NORMAL_PRODUCTS_SALE_ID, KFUIM_SALE_ID],
      branchIds: [],
      paymentMethod: [],
      status: ["paid"],
      createdVia: [],
    ...
    const kfuimOKFiltersObject = {
      searchTerm: "",
      saleIds: [KFUIM_SALE_ID],
      branchIds: [],
      paymentMethod: [],
      status: ["ok"],
      createdVia: [],
    ...
    const normalOrders = await getOrders(
      paidFiltersObject,
      token,
      amountDaysToImport
    );
    const kfuimOrders = await getOrders(
      kfuimOKFiltersObject,
      token,
      amountDaysToImport
    );
    const orders = [...normalOrders, ...kfuimOrders];
    ...
    const paidOrderProducts = await getOrderProducts(
      paidFiltersObject,
      token,
      orderIds,
      amountDaysToImport
    );
    const kfuimOrderProducts = await getOrderProducts(
      kfuimOKFiltersObject,
      token,
      orderIds,
      amountDaysToImport
    );
    const orderProducts = [...paidOrderProducts, ...kfuimOrderProducts];
